...
print("")
ExtremelySufficientNavySeal@213456
#Team "The Brownies" - Jadan Joa, Ethan Shieh
#IntroCS pd1
#FP -- Incremental Game
#2019-06-13
...
title = [
    ...
    "CSS = Counter-Strike: Source", "thanks for playing!", "less finger strain"
    ]
...
body {
    ...
    font-family: "Segoe UI", Tahoma, sans-serif; font-size: 110%;
}
...
#-------TURN QUERY DICTIONARY INTO PYTHON DICTIONARY-----------------------------------------------------------------------
...
if len(querydict) < 1:
    ...
    #'<a href="https://lisa.stuy.edu/~eshieh10/fp/rats.py?s=0&timestamp=' + str(int(time.time())) + '&score=0&stage=0&one=0&two=0&three=0&four=0&five=0&key=010">initialize</a>'
...
#-------HTML PAGE GENERATION-----------------------------------------------------------------------------------------------
        ...
        html_str += '<section id="population">'
        html_str += "<u>Population</u><br>"
        html_str += "Population: <b>" + commanum(str(score)) + "     / " + commanum(str(lim[stage])) + "</b><br>" #prints new calculated score
        ...
        html_str += '<section id="recruitment">'
        ...
        html_str += '<section id="expansion">'
        html_str += "<u>Expansion</u><br>"
        ...
        if stage % 2 == 0:
            ...
            doubletime = 1
        html_str += 'increases your maximum population of rats<br>every 2 stages, all group rate increases will double<br><br>'
        html_str += 'group rate has doubled <b>' + str(stage // 2) + '</b> times<br>'
        html_str += 'group rate will double in <b>' + str(doubletime) + '</b> expansions<br><br><br>'
        html_str += '</section>'

        

        ###text stuff
        html_str += '<section id="situation">'
        html_str += "<u>Situation</u><br>"
        html_str += txt + "<br><br><br>"
        html_str += '</section>'



        ###easter egg!!!
        html_str += '<section id="else">'
        html_str += '<a href="info.html">info</a>'
        if one == 42 or two == 42 or three == 42 or four == 42:
            html_str += '<br><br><br><br><br>thanks for playing!!<br>made by Team "The Brownies" -- Jadan Joa, Ethan Shieh'
